...
const { app, ipcMain, shell, autoUpdater } = require('electron')
...
expressServer.get('/file', async (req, res) => {
    ...
    if (fileType == "2") {
        contentType = FILE_TYPE_CONTENT_TYPE[fileType];
        disposition = "attachment;filename=" + messageInfo.fileName;
    }
    res.setHeader("Access-Control-Allow-Origin", "*");
    res.setHeader('Content-Type', contentType);
    if (disposition) {
        res.setHeader('Content-Disposition', contentType);
    }
    //头像，缩略图，文件
...
const downloadUpdate = async (id, fileName) => {
    ...
    const token = store.getUserData("token");
    const config = {
        responseType: 'stream', headers: { 'Content-Type': 'multipart/form-data', "token": token }, onDownloadProgress(progress) {

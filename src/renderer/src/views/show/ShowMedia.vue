...
onMounted(() => {
  ...
  //校验文件返回
  window.ipcRenderer.on('checkFileCallback', (e, data) => {
    //文件下载完成回调
    console.log('ipc checkFileCallback', data)
    const item = allFileList.value.find((item) => {
...
const getCurrentFile = () => {
  ...
  if (curFile.status != 1) {
    //文件未下载，请求后端下载文件
    window.ipcRenderer.send('checkFile', curFile.fileId)
    return
  }
  partType.value = curFile.partType

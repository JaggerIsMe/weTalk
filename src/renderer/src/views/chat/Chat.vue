...
const search = () => {
  ...
  chatSessionList.value.forEach((item) => {
    if (item.contactName.includes(searchKey.value) || item.lastMessage.includes(searchKey.value)) {
      ...
      newData.contactName = newData.contactName.replace(regex, "<span class='highlight'>$1</span>")
      newData.lastMessage = newData.lastMessage.replace(regex, "<span class='highlight'>$1</span>")
...
const onLoadChatSession = () => {
  window.ipcRenderer.on('loadSessionData', (e, data) => {
    //设置未读数
    let noReadCount = 0
...
const onLoadContactApply = () => {
  window.ipcRenderer.on('loadContactApply', (e, contactNoRead) => {
...
//删除会话
const delChatSession = (contactId, delDb) => {
